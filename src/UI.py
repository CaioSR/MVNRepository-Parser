...
class Interface:
  ...
  finished, should you choose to do so.""", font='arial 10', justify=LEFT)
        ...
        load_op_button = Button(buttons_frame, anchor='nw', width=20, text="Load operation", command=lambda : self.loadOP())
    ...
    def iniateNewOP(self):
        ...
        confirm_button = Button(self.ui, width=20, text="Begin Parse", command=lambda : self.finalize(project_input.get(), int(depth_input.get()), progress_dir_input.get(), final_dir_input.get()))
    ...
    def loadOP(self):
        ...
        self.finalize(project_url, max_depth, progress_dir, final_dir, repo = repo)
    ...
    def finalize(self, project, depth, prog_dir, final_dir, repo = None):
        self.setAttributes(project, depth, prog_dir, final_dir, repo)
        self.printAll()
        self.ui.destroy()
